...
async def main():
    async with async_playwright() as p:
        ...
        for width in (1440, 390):
            ...
            await expect(page.locator('.atlas-circle-svg [data-node-id]')).to_have_count(20)
            await expect(page.locator('.atlas-circle-svg [data-node-id="G-021"]')).to_have_count(0)
            await expect(page.locator('.atlas-unplaced summary')).to_contain_text('15')
            await page.locator('.atlas-circle-svg [data-node-id="G-004"] > a').first.focus()
            await page.keyboard.press('Enter')
            await expect(page.locator('#record-title')).to_have_text('Communications & Narrative Circle')
            ...
            await page.goto('http://localhost:8000/atlas#governance/circles/G-003')
            ...
            await expect(page.locator('.atlas-circle-breadcrumbs')).to_contain_text('Advocacy Circle')
            await page.locator('.atlas-circle-breadcrumbs a').first.click()
            await page.locator('.atlas-unplaced summary').click()
            await page.locator('.atlas-unplaced a[href="#governance/circles/G-021"]').click()
            await expect(page.locator('#record-title')).to_have_text('Advocacy Circle Lead')
